...
class User(UserMixin):
    ...
    def save(self): 
        newUser = user_model.User(email=self.email, password=self.password, active=self.active)
        newUser.save()
    ...
    def get_by_email(self, email):
    	...
    	dbUser = user_model.User.objects.get(email=email)
    	if dbUser:
    ...
    def get_mongo_doc(self):
        if self.id:
            return user_model.User.objects.with_id(self.id)
        else:
            return None
